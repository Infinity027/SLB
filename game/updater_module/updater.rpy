...
init -999 python in updater:
    ...
    def urlopen(url, headers=None):
        ...
        return io.BytesIO(data), status_code, resp


    def urlretrieve(url, fn, headers=None):
        import requests
    ...
    """
        ...
        """
            ...
            if (
            len(splitbase) >= 4
            and splitbase[-1] == "autorun"
            and splitbase[-2] == "Resources"
            and splitbase[-3] == "Contents"
            and splitbase[-4].endswith(".app")
        ):
        ...
        @property
        def download_total_mb(self):
            return int(round(self.download_total / 1024 / 1024, 0))
        
        @property
        def download_done_mb(self):
            return int(round(self.download_done / 1024 / 1024, 0))
        ...
        """
            ...
            try:
                ...
                renpy.persistent.authentication_token = False
                ...
                try:
                    self.debug_message = f"request:\n headers: {e.request.headers}\n url: {e.request.path_url}\nresponse:\n headers: {e.response.headers}\n content: {e.response.content}"
                    response_content = e.response.json()
                    if "failure" in response_content:
                        response_content = response_content.get("failure")
                    self.message = f"{e.__class__.__name__} {response_content.get('status')}: {response_content.get('cause')}"
                ...
                try:
                    response_details = e.response.headers.get("X-response-details")
                except AttributeError:
                    response_details = None
                if not response_details:
                    try:
                        response_details = e.response.headers.get("X-oauthresp")
                    except AttributeError:
                        response_details = None
                if response_details:
                    try:
                        cause = json.loads(response_details)
                        cause = cause.get("cause")
                        self.message = f"{e.response.reason.capitalize()}: {cause}"
                    except:
        ...
        """
            ...
            self.test_write()
            self.check_updates()
            if "failure" in self.updates or self.state in [self.ERROR, self.HTTPERROR]:
                return
        ...
        """
            ...
            try:
                ...
                raise UpdateError(
                __("Could not download file list: ") + str(e), response=e.response
            )
            
            if (
            hashlib.sha256(resp.content).hexdigest()
            != self.updates[module]["rpu_digest"]
        ):
            ...
            try:
                rv = FileList.from_json(json.loads(data), f"{module}_rpu")
            except:
                self.log.write(f"FileList.from_json(json.loads(data)): {data}")
                raise
            return rv
        ...
        """
            ...
            if os.environ.get("USE_DOWNLOADER"):
                if os.path.isfile(renpy.config.environment_path):
                    with open(renpy.config.environment_path, "r") as f:
                        environment_content = f.readlines()
                    environment_content = [
                    'USE_DOWNLOADER="OFF"' if "USE_DOWNLOADER" in i else i
                    for i in environment_content
                ]
                else:
                    environment_content = ['USE_DOWNLOADER="OFF"']
                with open(renpy.config.environment_path, "w") as f:
                    f.writelines(environment_content)
        ...
        """
            ...
            if (
            self.state == self.UPDATE_NOT_AVAILABLE
            or self.state == self.ERROR
            or self.state == self.HTTPERROR
        ...
        """
            ...
            if os.path.exists(path):
                ...
                DELETED, os.path.basename(path) + "." + str(random.randint(0, 1000000))
            )
        ...
        def test_write(self):
            ...
            try:
                ...
                raise UpdateError(
                _("This account does not have permission to perform an update.")
            )
            
            if not self.log:
                raise UpdateError(
                _("This account does not have permission to write the update log.")
            )
        ...
        """
            ...
            response_code, resp = urlretrieve(self.url, fn, self.headers)
            
            if response_code in [401, 403, 404, 500]:
                self.updates = {"failure": f"{response_code} on {self.url}"}
                self.message = f"HTTP {response_code} {self.url}"
                raise HTTPError(self.message)
            ...
            if os.path.exists(key):
                ...
                try:
                    ...
                    f, response_code, resp = urlopen(url, self.headers)
                    if response_code in [401, 403, 404, 500]:
                        self.updates = {"failure": f"{response_code} on {self.url}"}
                        self.message = f"HTTP {response_code} {self.url}"
                        raise HTTPError(self.message)
            ...
            if self.public_key is not None:
                ...
                try:
                    ...
                    response_code, resp = urlretrieve(self.url + ".sig", fn, self.headers)
                    if response_code in [401, 403, 404, 500]:
                        self.updates = {"failure": f"{response_code} on {self.url}"}
                        self.message = f"HTTP {response_code} {self.url}"
                        raise HTTPError(self.message)
            ...
            try:
                ...
                self.log.write(f"Fail to json.loads(updates_json): {updates_json}")
                raise
        ...
        def add_dlc_state(self, name):
            ...
            if has_rpu:
                ...
                f, response_code, resp = urlopen(url, headers=self.headers)
                if response_code in [401, 403, 404, 500]:
                    self.updates = {"failure": f"{response_code} on {self.url}"}
                    self.message = f"HTTP {response_code} {self.url}"
                    raise HTTPError(self.message)
        ...
        """
            ...
            f, response_code, resp = urlopen(
            urlparse.urljoin(self.url, self.updates[module]["sums_url"]),
            headers=self.headers,
        )
            if response_code in [401, 403, 404, 500]:
                self.updates = {"failure": f"{response_code} on {self.url}"}
        ...
        """
            ...
            if not resp.ok:
                raise UpdateError(_("The update file was not downloaded."), response=resp)
        ...
        """
            ...
            with tarfile.open(update_fn, "r") as tf:
                for i, info in enumerate(tf):
                    ...
                    if not info.isreg():
                        raise UpdateError(
                        __("While unpacking {}, unknown type {}.").format(
                            info.name, info.type
                        )
                    )
    ...
    """
        ...
        ui.timer(0.1, repeat=True, action=u.periodic)
    ...
    def update_command():
        ...
        "--base",
        action="store",
        help="The base directory of the game to update. Defaults to the current game.",
    )
        ap.add_argument(
        "--force",
        action="store_true",
        help="Force the update to run even if the version numbers are the same.",
    )
        ap.add_argument(
        "--key",
        action="store",
        help="A file giving the public key to use of the update.",
    )
        ap.add_argument(
        "--simulate",
        help="The simulation mode to use. One of available, not_available, or error.",
    )
        
        args = ap.parse_args()
        
        u = Updater(
        args.url, args.base, args.force, public_key=args.key, simulate=args.simulate
    )
    ...
    """
        ...
        while True:
            ui.timer(0.1, repeat=True, action=downloader.periodic)
            renpy.call_screen(screen, u=downloader)
            
            downloader = Updater(confirm=False, **downloader_kwargs)
            
            print("sending websocket update...")
            renpy.websocket.sio.emit("ping")
            print("sending websocket update... with renpy")
            renpy.websocket.sio.emit("ping")
