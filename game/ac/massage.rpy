...
label massage:
    ...
    if renpy.has_label(f"{active_girl.id}_massage_activity_male"):
        ...
        if renpy.has_label(f"{active_girl.id}_massage_intro_male"):
            call expression f"{active_girl.id}_massage_intro_male" from _call_expression_121
        else:
            "I massage [active_girl.name]."
        if hero.fitness + hero.charm >= active_girl.love:
            $ renpy.show(f"{active_girl.id} happy")
            if renpy.has_label(f"{active_girl.id}_massage_accept_male"):
                call expression f"{active_girl.id}_massage_accept_male" from _call_expression_122
            $ gain = 1
            ...
            if renpy.has_label(f"{active_girl.id}_massage_refuse_male"):
