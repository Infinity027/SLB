    ...
    if result != "None" and result != "exit":
        ...
        if (hero.charm >= difficulty - active_girl.love) or from_birthday_date:
            ...
            if (hero.activity and not hero.activity.flags.canceled) or not hero.activity:
                if not from_birthday_date:
                    if game.calendar.is_today(*active_girl.birthday) and renpy.has_label(f"{active_girl.id}_birthday_gift_male"):
                        call expression f"{active_girl.id}_birthday_gift_male" from _call_expression_19
                    elif game.calendar.is_today("valentine") and renpy.has_label(f"{active_girl.id}_valentine_gift_male"):
                        call expression f"{active_girl.id}_valentine_gift_male" from _call_expression_40
                    elif game.calendar.is_today("christmas") and renpy.has_label(f"{active_girl.id}_christmas_gift_male"):
                        call expression f"{active_girl.id}_christmas_gift_male" from _call_expression_41
...
label give_a_gift_reaction(active_npc, reaction_result):
    ...
    if reaction_result == "neutral":
        if renpy.has_label(f"{active_npc.id}_give_a_gift_reaction_neutral_male") and not active_npc.flags.neutral_gift_reaction:
            call expression f"{active_npc.id}_give_a_gift_reaction_neutral_male" from _call_expression_36
        ...
        if renpy.has_label(f"{active_npc.id}_give_a_gift_reaction_positive_male") and not active_npc.flags.positive_gift_reaction:
            call expression f"{active_npc.id}_give_a_gift_reaction_positive_male" from _call_expression_55
            $ active_npc.flags.positive_gift_reaction = TemporaryFlag(True, "week")
        else:
            "[active_girl.name] seems to enjoy the gift."
    else:
        if renpy.has_label(f"{active_npc.id}_give_a_gift_reaction_negative_male") and not active_npc.flags.negative_gift_reaction:
            call expression f"{active_npc.id}_give_a_gift_reaction_negative_male" from _call_expression_56
...
label npc_give_a_gift:
    ...
    if game.calendar.is_today(*hero.birthday) and active_girl.love >= 40 and renpy.has_label(f"{active_girl.id}_give_birthday_male"):
        call expression f"{active_girl.id}_give_birthday_male" from _call_expression_69
    elif game.calendar.is_today("valentine") and active_girl.love >= 100 and renpy.has_label(f"{active_girl.id}_give_valentine_male"):
        call expression f"{active_girl.id}_give_valentine_male" from _call_expression_71
